...
function Donut({ good, damaged, empty, size = 90 }) {
  ...
  return (
    <div style={{ textAlign: 'center' }}>
      ...
      <div style={{ fontSize: 12, color: C.muted, marginTop: 8 }}>Good Condition</div>
    </div>
  );
}
...
function StatusCard({ title, good, damaged, empty, total, accent }) {
  return (
    <div style={{ background: C.surface, border: `1px solid ${C.line}`, borderRadius: 16, padding: 20, borderTop: `4px solid ${accent}` }}>
...
function TechActivityTable({ data }) {
  ...
  if (activities.length === 0) {
    return (
      <div style={{ background: C.surface, border: `1px solid ${C.line}`, borderRadius: 16, padding: 40, textAlign: 'center', color: C.muted }}>
  ...
  return (
    <div style={{ background: C.surface, border: `1px solid ${C.line}`, borderRadius: 16, overflow: 'hidden' }}>
      <div style={{ padding: 20, fontWeight: 700, borderBottom: `1px solid ${C.line}` }}>
      ...
      <div style={{ maxHeight: 360, overflow: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
          <thead style={{ position: 'sticky', top: 0, background: '#f8fafc' }}>
            <tr>
              <th style={{ padding: '14px 20px', textAlign: 'left' }}>Technician</th>
              <th style={{ padding: '14px 20px', textAlign: 'left' }}>Date</th>
              <th style={{ padding: '14px 20px', textAlign: 'left' }}>Time</th>
            </tr>
          </thead>
          <tbody>
            {activities.map((row, i) => (
              <tr key={i} style={{ borderTop: `1px solid ${C.line}` }}>
                <td style={{ padding: '14px 20px', fontWeight: 700, color: C.blue }}>
                  {row.userName}
                </td>
                <td style={{ padding: '14px 20px', color: C.muted }}>{row.date || '—'}</td>
                <td style={{ padding: '14px 20px', color: C.muted }}>{row.time || '—'}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
export default function Dashboard() {
  ...
  if (loading) return <div style={{ background: C.bg, minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>Loading Dashboard...</div>;
    ...
    <div style={{ background: C.bg, minHeight: '100vh' }}>
      <div style={{ maxWidth: '1600px', margin: '0 auto', padding: '24px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
          <h1 style={{ margin: 0, fontSize: 28, fontWeight: 700, color: C.navy }}>Data Center Asset Management</h1>
          <button onClick={fetchData} style={{ padding: '10px 20px', background: C.navy, color: '#fff', border: 'none', borderRadius: 8, cursor: 'pointer' }}>
            Refresh
          </button>
        </div>

        {/* Filters */}
        <div style={{ background: C.surface, padding: 20, borderRadius: 16, border: `1px solid ${C.line}`, marginBottom: 28, display: 'flex', gap: 16, flexWrap: 'wrap', alignItems: 'center' }}>
          <input
            type="text"
          ...
          />
          <span style={{ color: C.muted, fontWeight: 500 }}>Scan Date:</span>
          <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)} min={allDates[0]} style={{ padding: '9px 12px', borderRadius: 8, border: `1px solid ${C.line}` }} />
          <span style={{ color: C.muted }}>to</span>
          <input type="date" value={dateTo} onChange={e => setDateTo(e.target.value)} max={allDates[allDates.length - 1]} style={{ padding: '9px 12px', borderRadius: 8, border: `1px solid ${C.line}` }} />

          {(searchTerm || dateFrom || dateTo) && (
            <button onClick={() => { setSearchTerm(''); setDateFrom(''); setDateTo(''); }} style={{ color: C.red }}>Clear</button>
          )}
        </div>
        ...
        {st && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 20, marginBottom: 32 }}>
            <div style={{ background: `linear-gradient(135deg, ${C.navy}, #1e293b)`, borderRadius: 20, padding: 24, color: '#fff' }}>
              <div style={{ fontSize: 14, opacity: 0.9 }}>Total Assets</div>
              <div style={{ fontSize: 48, fontWeight: 800, marginTop: 8 }}>{st.total}</div>
            </div>
          ...
          {st && (
            <div style={{ background: C.surface, border: `1px solid ${C.line}`, borderRadius: 16, padding: 24 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 32 }}>
                <Donut good={st.condGood} damaged={st.condDmg} empty={st.condEmpty} size={110} />
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', gap: 24, marginBottom: 16 }}>
                    <div><div style={{ fontSize: 32, fontWeight: 800, color: C.green }}>{st.condGood}</div><div style={{ fontSize: 12, color: C.muted }}>Good</div></div>
                    <div><div style={{ fontSize: 32, fontWeight: 800, color: C.red }}>{st.condDmg}</div><div style={{ fontSize: 12, color: C.muted }}>Damaged</div></div>
                    <div><div style={{ fontSize: 32, fontWeight: 800, color: C.slate }}>{st.condEmpty}</div><div style={{ fontSize: 12, color: C.muted }}>Empty</div></div>
                  </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
          <div>
            {st && <div style={{ marginBottom: 24 }}><StatusCard title="⚡ PSU-1 Status" good={st.psu1Good} damaged={st.psu1Dmg} empty={st.psu1Empty} total={st.total} accent={C.violet} /></div>}
            {st && <StatusCard title="⚡ PSU-2 Status" good={st.psu2Good} damaged={st.psu2Dmg} empty={st.psu2Empty} total={st.total} accent={C.cyan} />}
          </div>
